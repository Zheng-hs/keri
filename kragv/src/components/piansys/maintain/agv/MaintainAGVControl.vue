      ...
      <el-row :gutter="20"
        ...
        <el-col class="btnrow1">
          ...
          <el-button type="primary"
                     @click="agvStartControl(selectedRowId)">启动</el-button>
          <el-button type="primary"
                     @click="agvStopControl(selectedRowId)">停止</el-button>
          <el-button type="primary"
                     @click="agvRestorationControl(selectedRowId)">复位</el-button>
        </el-col>
...
export default {
  ...
  methods: {
    ...
    // AGV启动
    async agvStartControl (id) {
      if (id === '') {
        return this.$message.info('请选择AGV！')
      }
      // console.log(id)
      // 弹框提示用户是否删除信息
      const confirmResult = await this.$confirm('此操作将启动AGV, 是否继续?', '提示', {
      ...
      this.controlParams.agvId = id
      this.controlParams.cmd = 'Go'
      const { data: res } = await this.$http.post('maintain/agvc/control/', this.controlParams)
      this.controlParams = {}
      if (res.meta.status !== 200) {
        return this.$message.error('AGV启动失败！')
      }

      this.$message.success('AGV启动成功！')
      this.getMonitorAGVList()
    },
    // AGV停止
    async agvStopControl (id) {
      if (id === '') {
        return this.$message.info('请选择AGV！')
      }
      // console.log(id)
      // 弹框提示用户是否删除信息
      const confirmResult = await this.$confirm('此操作将停止AGV, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).catch(err => err)
      ...
      this.controlParams.agvId = id
      this.controlParams.cmd = 'Stop'
      const { data: res } = await this.$http.post('maintain/agvc/control/', this.controlParams)
      this.controlParams = {}
      if (res.meta.status !== 200) {
        return this.$message.error('AGV停止失败！')
      }

      this.$message.success('AGV停止成功！')
      this.getMonitorAGVList()
    },
    // AGV复位
    async agvRestorationControl (id) {
      if (id === '') {
        return this.$message.info('请选择AGV！')
      }
      // console.log(id)
      // 弹框提示用户是否删除信息
      const confirmResult = await this.$confirm('此操作将复位AGV, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).catch(err => err)
      ...
      this.controlParams.agvId = id
      this.controlParams.cmd = 'Reset'
      const { data: res } = await this.$http.post('maintain/agvc/control/', this.controlParams)
      this.controlParams = {}
      if (res.meta.status !== 200) {
        return this.$message.error('AGV复位失败！')
      }

      this.$message.success('AGV复位成功！')
      this.getMonitorAGVList()
    }
