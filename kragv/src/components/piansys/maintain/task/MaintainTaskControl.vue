      ...
      <el-row :gutter="20"
        ...
        <el-col class="btnrow1">
          ...
          <el-button type="primary"
                     @click="cancelTaskById(selectedMissionId)">任务取消</el-button>
          <el-button type="primary"
                     @click="taskToTop(selectedMissionId)">任务顶置</el-button>
          <el-button type="primary"
                     @click="showTaskSourceDialog">任务来源</el-button>
        </el-col>
...
export default {
  ...
  methods: {
    ...
    // 取消任务
    async cancelTaskById (id) {
      if (id === '') {
        return this.$message.info('请选择任务！')
      }
      // 弹框提示用户是否删除信息
      const confirmResult = await this.$confirm('此操作将取消该条任务, 是否继续?', '提示', {
      ...
      const { data: res } = await this.$http.delete('maintain/mission/', { data: { missionId: id } })
      if (res.meta.status !== 200) {
        return this.$message.error('任务取消失败！')
      }
    ...
    async taskToTop (id) {
      ...
      // 弹框提示用户是否删除信息
      const confirmResult = await this.$confirm('此操作将顶置该条任务, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).catch(err => err)
      ...
      const { data: res } = await this.$http.put('maintain/mission/', { missionId: id })
      if (res.meta.status !== 200) {
        return this.$message.error('任务顶置失败！')
      }

      this.$message.success('任务顶置成功！')
      this.getMonitorTaskList()
    },
    // 打开任务来源对话框 (暂时无用)
    showTaskSourceDialog () {
      return this.$message.info('该功能暂未开放')
    }
